...
export class ApiService {
    ...
    private urlFor(controller: string) {
        return `http://api.one-equality.com/v1/private/blog/${controller}`;
    }

    private headers(headerType: string = 'json'): HttpHeaders {
        const token = ''; // change to let
        // this.store.select(state => state.auth.tokenInfo.authToken).pipe(
        //     filter(tokenInfo => !!tokenInfo),
        //     take(1)
        // ).subscribe(tokenInfo => token = tokenInfo);
